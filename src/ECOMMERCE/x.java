...
public class HOME {
	...
	ResultSet tables1 = dbm.getTables(null, null, "cart", null);
	if (!tables1.next()) {
	String sql = "create table cart " + " (prod_id int , " + " prod_quantity int ,"+ "customer_id  int NOT NULL )";
	stmt.executeUpdate(sql);
	stmt.executeUpdate("ALTER TABLE cart add primary key(prod_id, customer_id)");
	stmt.executeUpdate(	"Alter table cart ADD CONSTRAINT cart_productid FOREIGN KEY(prod_id) REFERENCES product(product_id)");
	}
		} catch (Exception e) {
		}
		;
		...
		mb.add(cate);
		seller_signup_btn = new JButton("Seller_SignUp");
		seller_signup_btn.setBackground(Color.white);
		seller_signup_btn.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				f.dispose();
		...
		seller_signup_btn.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR)); // Changing the cursor when hovered
																												// to home button
		seller_signup_btn.addMouseListener(new MouseAdapter() {
			public void mouseEntered(MouseEvent evt) {
				seller_signup_btn.setBackground(Color.ORANGE);
			}
		...
		mb.add(tf1);
		ImageIcon imageIcon = new ImageIcon("C:\\Users\\lokesh\\search.png");
   ...
   JPanel p1 = d.getProudctsPanel(prodType, gender);
	...
	int prod_quantity = 0;
	int i = 0;
	ResultSet rs2 = stmt.executeQuery("select prod_quantity,prod_name from product where prod_id = " + product_id + "");
	String name = "";
	if (rs2.next()) {
	prod_quantity = rs2.getInt(1);
	name = rs2.getString(2);
	}
	ResultSet rs3 = stmt.executeQuery("select*from cart where prod_id = " + product_id+ " and customer_id = " + Global.ID + "");
   if (rs3.next()) {
	int quantity = rs3.getInt(2);
	int reply = 0;
	if (prod_quantity > 0) {
	if (quantity < prod_quantity)
	reply = JOptionPane.showConfirmDialog(null, name+ "  already exists in cart.Would you like to increase the quantity? ","delete", JOptionPane.YES_NO_OPTION);
	else
	JOptionPane.showMessageDialog(null, "No products available");
	if (reply == JOptionPane.YES_OPTION) {
	if (quantity < prod_quantity) {
	quantity += 1;
	stmt.executeUpdate("update cart set prod_quantity = " + (quantity) + "  where prod_id = "+ product_id + " and  customer_id = " + Global.ID + " ");
   }
	}
	}
	else JOptionPane.showMessageDialog(null, "No products available");
	}
	else {
	if (prod_quantity > 0) {
	String sql = "insert into cart(prod_id,prod_quantity,customer_id) VALUES ("+ product_id + "," + (i + 1) + "," + Global.ID + " )";
	i++;
	prod_quantity--;
	stmt.executeUpdate(sql);
